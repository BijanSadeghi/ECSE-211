...
public class WallFollowingLab {
	...
	private static final int motorLow = 200; // Speed of slower rotating wheel
												// (deg/sec)
	private static final int motorHigh = 300; // Speed of the faster rotating
												// wheel (deg/seec)	

	private static final Port usPort = LocalEV3.get().getPort("S1");
	public static SensorModes usSensor = new EV3UltrasonicSensor(usPort); // usSensor is
																				// the instance
	public static SampleProvider usDistance = usSensor.getMode("Distance"); // usDistance
																// provides
																// samples
																// from
																// this
																// instance
	public static float[] usData = new float[usDistance.sampleSize()]; // usData is the
															// buffer in
															// which data
															// are
															// returned
	...
	public static void main(String[] args) {
		...
		while (option == 0) // and wait for a button press. The button
			option = Button.waitForAnyPress(); // ID (option) determines what
		...
		// 1. Create a port object attached to a physical port (done already
		// above)
		...
		UltrasonicPoller usPoller = null; // the selected controller on each
											// cycle

		// Depending on which button was pressed, invoke the US poller and
		// printer with the
		...
		case Button.ID_RIGHT: // Proportional control selected
			...
			break;
		default:
			System.out.println("Error - invalid button"); // None of the above -
															// abort
			System.exit(-1);
			break;
		}
